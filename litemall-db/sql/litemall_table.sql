...
CREATE TABLE `litemall_order` (
  ...
  `pay_type` tinyint(3) NOT NULL DEFAULT '0' COMMENT '支付方式，= 0 未知, 1 支付宝, 2 微信, 3 线下支付',
  ...
  `invoice_url` varchar(255)  NULL DEFAULT '' COMMENT '发票',
  `pay_voucher_url` varchar(255)  NULL DEFAULT '' COMMENT '付款凭证',
...
CREATE TABLE `litemall_order_goods` (
  ...
  `exp_date` datetime DEFAULT NULL COMMENT '商品失效期',
   ...
   `max_register_users_count` smallint(5) DEFAULT '0' COMMENT '最大绑定用户数',
   `has_register_user_ids` varchar(255) DEFAULT '[]' COMMENT '已绑定用户ID列表',
...
CREATE TABLE `litemall_user` (
  ...
  `session_key` varchar(255) NULL DEFAULT '' COMMENT '微信登录会话KEY',
  `status` tinyint(3) NOT NULL DEFAULT '0' COMMENT '0 可用, 1 禁用, 2 注销',
  `add_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  `deleted` tinyint(1) DEFAULT '0' COMMENT '逻辑删除',
